...
def is_sort_message(message):
    ...
    return (
        "message_type" in message and
        message["message_type"] == "new_sort_task"
    )
...
def wait_for_messages(function, mock_socket, num=1, block=True):
...
def wait_for_reduce_messages(mock_socket, num=1, block=True):
    """Return after num map messages."""
    return wait_for_messages(is_reduce_message, mock_socket, num, block=block)


def wait_for_sort_messages(mock_socket, num=1, block=True):
    """Return after num sort messages."""
    return wait_for_messages(is_sort_message, mock_socket, num, block=block)


def wait_for_log(caplog, match_str):
    """Check for match_str in master logging msgs."""
    for _ in range(TIMEOUT):
        if match_str in caplog.text:
            return True
        time.sleep(1)

    raise Exception("Did not find '{}' in logs".format(
        match_str
    ))
