...
def master_message_generator(mock_socket):
    ...
    os.mkdir("tmp/test_worker_04/output")

    # New map job
    yield json.dumps({
        'message_type': 'new_worker_task',
        'executable': TESTDATA_DIR/'exec/wc_map.sh',
        'input_files': [TESTDATA_DIR/'input/file02'],
...
def test_worker_04_job(mocker):
    ...
    """
    utils.create_and_clean_testdir("test_worker_04")
    ...
    assert messages == [
        ...
        },
        {
            "message_type": "status",
            "output_files": [
                "tmp/test_worker_04/output/file02"
            ],
            "status": "finished",
            "worker_pid": os.getpid(),
        },
    ...
    # Verify final output
    with Path('tmp/test_worker_04/output/file02').open() as infile:
