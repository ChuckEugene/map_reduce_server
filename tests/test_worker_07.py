...
def test_worker_07_two_inputs(mocker):
    ...
    with Path("tmp/test_worker_07/file01").open() as infile:
        mapout01 = infile.readlines()
    with Path("tmp/test_worker_07/file02").open() as infile:
    ...
    with Path("tmp/test_worker_07/group.out").open() as infile:
