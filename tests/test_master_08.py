...
def worker_message_generator(mock_socket, memory_profiler, master_log):
    ...
    utils.wait_for_log(master_log, "end group stage")
...
def test_master_08_roundrobin(mocker, caplog):
    ...
    assert filecmp.cmp(
        ...
        TESTDATA_DIR/"test_master_08/correct/job-0/grouper-output/reduce01",
        shallow=False
    )
    assert filecmp.cmp(
        "./tmp/job-0/grouper-output/reduce02",
        TESTDATA_DIR/"test_master_08/correct/job-0/grouper-output/reduce02",
        shallow=False
    )
