...
def master_message_generator(mock_socket):
    ...
    yield json.dumps({
        ...
        'output_directory': "tmp/test_worker_08",
        'worker_pid': os.getpid(),
    ...
    yield json.dumps({
        ...
        'input_files': [
            'tmp/test_worker_08/file01',
            'tmp/test_worker_08/file02',
        ],
        'output_file': "tmp/test_worker_08/group.out",
        'worker_pid': os.getpid()
    }).encode('utf-8')
    yield None

    # Simulate master creating file reduce1
    Path("tmp/test_worker_08/reduce01").write_text(
        "\t1\n"
        "\t1\n"
        "bye\t1\n"
        "goodbye\t1\n"
    ...
    # Wait for worker to finish sort job.  There should now be three
    # status=finished messages in total.  One from the previous map task, one
    # from the previous sort task, and one from this reduce task.
...
def test_worker_08_map_group_reduce(mocker):
    """Verify worker can map, group, and reduce.
    ...
    assert messages == [
        ...
        },
        {
            "message_type": "status",
            "output_files": [
                "tmp/test_worker_08/output/reduce01",
            ],
            "status": "finished",
            "worker_pid": os.getpid(),
        },
    ...
    with Path("tmp/test_worker_08/file01").open() as infile:
        mapout01 = infile.readlines()
    with Path("tmp/test_worker_08/file02").open() as infile:
    ...
    # Validate group stage output
    with Path("tmp/test_worker_08/group.out").open() as infile:
    ...
    # Validate reduce stage output
    with Path('tmp/test_worker_08/output/reduce01').open() as infile:
