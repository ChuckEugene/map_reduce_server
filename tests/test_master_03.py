...
def worker_message_generator(mock_socket):
    ...
    yield json.dumps({
        ...
        'output_directory': "tmp/test_master_03/output",
    ...
    distutils.dir_util.copy_tree(
        TESTDATA_DIR/"test_master_03/intermediate/job-0",
        "tmp/job-0"
    )
    ...
    utils.wait_for_isfile("tmp/test_master_03/output/outputfile01")
...
def test_master_03_finish(mocker):
    """Verify master finishes job correctly.

    Note: 'mocker' is a fixture function provided the the pytest-mock package.
    This fixture lets us override a library function with a temporary fake
    function that returns a hardcoded value while testing.
    """
    utils.create_and_clean_testdir("test_master_03")
    ...
    assert filecmp.cmp(
        "tmp/job-0/grouper-output/reduce01",
        TESTDATA_DIR/'test_master_03/correct/job-0/grouper-output/reduce01',
        shallow=False
    )
