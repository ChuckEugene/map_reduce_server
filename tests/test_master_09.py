...
def worker_message_generator(mock_socket):
    ...
    yield json.dumps({
        ...
        'output_directory': "tmp/test_master_09/output",
    ...
    distutils.dir_util.copy_tree(
        TESTDATA_DIR/"test_master_09/intermediate/job-0",
        "tmp/job-0"
    )
    ...
    # Wait for Master to assign 2nd map message
...
def test_master_09_midstage_register(mocker):
    """Verify master will allocate tasks to workers registering mid-stage.

    Note: 'mocker' is a fixture function provided the the pytest-mock package.
    This fixture lets us override a library function with a temporary fake
    function that returns a hardcoded value while testing.
    """
    utils.create_and_clean_testdir("test_master_09")
