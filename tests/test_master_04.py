...
def worker_message_generator(mock_socket):
    ...
    yield json.dumps({
        ...
        'output_directory': "tmp/test_master_04/output0",
    ...
    yield json.dumps({
        'message_type': 'new_master_job',
        'input_directory': TESTDATA_DIR/'input/',
        'output_directory': 'tmp/test_master_04/output1',
        'mapper_executable': TESTDATA_DIR/'exec/wc_map.sh',
        'reducer_executable': TESTDATA_DIR/'exec/wc_reduce.sh',
        'num_mappers': 2,
        'num_reducers': 1
    ...
    distutils.dir_util.copy_tree(
        TESTDATA_DIR/"test_master_04/intermediate/job-0",
        "tmp/job-0"
    )
    distutils.dir_util.copy_tree(
        TESTDATA_DIR/"test_master_04/intermediate/job-1",
        "tmp/job-1"
    )
    ...
    # Wait for Master to send the 3rd map message (counting both job-0, job-1)
    for _ in utils.wait_for_map_messages(mock_socket, num=3, block=False):
        yield None
    ...
    # Wait for Master to send the 4th map message (total from both jobs)
    ...
    # Wait for Master to send sort job messages.  We expect two messages
    ...
    # Wait for Master to send reduce job message.  We expect two messages
    # because one is from the previous job 0.
    ...
    utils.wait_for_isfile("tmp/test_master_04/output1/outputfile01")
...
def test_master_04_multiple(mocker):
    """Verify master can handle multiple jobs.

    Note: 'mocker' is a fixture function provided the the pytest-mock package.
    This fixture lets us override a library function with a temporary fake
    function that returns a hardcoded value while testing.
    """
    utils.create_and_clean_testdir("test_master_04")
    ...
    assert filecmp.cmp(
        ...
        TESTDATA_DIR/"test_master_04/correct/job-0/grouper-output/reduce01",
        shallow=False
    )
    ...
    assert filecmp.cmp(
        "tmp/job-1/grouper-output/reduce01",
        TESTDATA_DIR/"test_master_04/correct/job-1/grouper-output/reduce01",
        shallow=False
    )

    # Verify final output files were created
    assert os.path.exists("tmp/test_master_04/output0/outputfile01")
